# CASA — Board editoriale della presentazione

> Documento operativo per discutere, approvare e tracciare tutte le modifiche.
> `leggimi.html`, `llms.txt` e la presentazione interattiva sono output: verranno aggiornati solo dopo l'approvazione dei contenuti in questo board.

## Come si usa

Per ogni pagina sono disponibili:

- **stato**;
- **funzione narrativa**;
- **priorità da presidiare**;
- **testo corrente o sintesi**;
- **problemi aperti**;
- **proposta**;
- **decisioni e osservazioni**.

Stati:

- `DA RIVEDERE` — non ancora affrontata;
- `IN PROPOSTA` — esiste una nuova versione da discutere;
- `DA VERIFICARE` — testo approvato, in attesa del controllo trasversale;
- `APPROVATA` — contenuto definitivo;
- `APPENDICE` — esclusa dal racconto principale ma conservata come approfondimento.

## Priorità trasversali

| Codice | Priorità |
|---|---|
| P1 | Ambiente, agricoltura, cibo e salute restano visibili attraverso relazioni concrete tra fenomeni, dati, pratiche e risultati; non basta ripeterne i nomi. |
| P2 | CASA richiede uno spazio fisico comune; la localizzazione non è ancora individuata. |
| P3 | Lo spazio è sperimentale: aree aperte, serre, campi sensorizzati, laboratori modulari, formazione ed eventi. |
| P4 | L'Internet delle risorse amplia lo spazio comune attraverso le capacità dei partner, senza sostituirlo. |
| P5 | CASA non è un ecosistema generico né un catalogo di progetti. |
| P6 | Innovazione e tecnologie digitali sono abilitatori trasversali, non un catalogo tecnologico. |
| P7 | Ruoli, impegni e stato dei partner sono descritti con precisione istituzionale. |
| P8 | La presentazione mantiene ritmo, essenzialità e una progressione senza ripetizioni. |

## Indice e stato

| ID | Pagina | Blocco | Stato | Intervento previsto |
|---:|---|---|---|---|
| 01 | Copertina | A · Problema | DA RIVEDERE | Modifica lieve |
| 02 | Il problema | A · Problema | DA RIVEDERE | Mantieni con raccordo |
| 03 | Gli effetti attraversano gli ambiti | A · Problema | DA RIVEDERE | Modifica lieve |
| 04 | Cosa manca davvero? | B · Identità | APPROVATA | Modifica sostanziale |
| 05 | Per questo nasce CASA | B · Identità | APPROVATA | Modifica sostanziale |
| 06 | Lo spazio CASA | B · Identità | APPROVATA | Nuova pagina |
| 07 | Internet delle risorse | B · Identità | DA VERIFICARE | Modifica sostanziale |
| 08 | Quattro ambiti che lavorano insieme | B · Identità | DA RIVEDERE | Nuova funzione narrativa |
| 09 | Come opera CASA | C · Funzionamento | DA RIVEDERE | Modifica sostanziale |
| 10 | Per chi è CASA e benefici | C · Funzionamento | DA RIVEDERE | Modifica mirata/fusione |
| 11 | Ecosistema e ruoli | D · Credibilità | DA RIVEDERE | Ristrutturazione |
| 12 | CASA non parte da zero | D · Credibilità | DA RIVEDERE | Accorcia/fondi |
| 13 | Roadmap | D · Credibilità | DA RIVEDERE | Modifica sostanziale |
| 14 | Direzione scientifica | D · Credibilità | DA RIVEDERE | Correzione mirata |
| 15 | Chiusura | D · Credibilità | DA RIVEDERE | Modifica sostanziale |
| A1 | Accordo UNIUST–Tuscia | Approfondimenti | APPENDICE | Mantieni autonoma |
| A2 | Università di Malta | Approfondimenti | APPENDICE | Mantieni autonoma |
| A3 | Europa, Mediterraneo e Piano Mattei | Approfondimenti | APPENDICE | Verifica istituzionale |
| A4 | Proposta InnovaliaTech | Add-on | APPENDICE | Da sviluppare separatamente |

---

## Blocco A — Il problema

### 01 · Copertina

**Stato:** `DA RIVEDERE`  
**Funzione:** identità e posizionamento.  
**Priorità:** P1, P3, P5, P8.

**Testo corrente**

> CASA  
> Cibo · Ambiente · Salute · Agricoltura  
> L'infrastruttura condivisa che abilita l'innovazione tra settori.

**Problemi aperti**

- “Infrastruttura condivisa” è corretta ma ancora generica.
- Non deve diventare una copertina descrittiva o sovraccarica.

**Proposta:** da scrivere dopo l'approvazione del blocco centrale.

**Decisioni e osservazioni:** —

### 02 · Il problema

**Stato:** `DA RIVEDERE`  
**Funzione:** introdurre l'interdipendenza.  
**Priorità:** P1, P8.

**Testo corrente**

> L'ambiente condiziona l'agricoltura.  
> L'agricoltura dà forma al cibo che produciamo.  
> Il cibo incide sulla nostra salute.  
> Eppure continuiamo a organizzare competenze, decisioni e interventi separatamente.

**Problemi aperti**

- La catena funziona, ma appare lineare: manca il ritorno delle evidenze sul sistema.

**Proposta:** mantenere il testo e valutare un raccordo visivo o conclusivo che anticipi il ciclo.

**Decisioni e osservazioni:** —

### 03 · Gli effetti attraversano gli ambiti

**Stato:** `DA RIVEDERE`  
**Funzione:** dimostrare quantitativamente l'interdipendenza.  
**Priorità:** P1, P8.

**Testo corrente:** tre evidenze su emissioni agroalimentari, produttività agricola e costi nascosti dei sistemi agroalimentari.

**Problemi aperti**

- Deve preparare la necessità di osservare e sperimentare il sistema nello stesso ambiente.
- I numeri devono essere letti come passaggi tra ambiti, non come tre statistiche indipendenti.

**Proposta:** da sviluppare dopo il blocco centrale.

**Decisioni e osservazioni:** —

---

## Blocco B — Identità di CASA

### 04 · Cosa manca davvero?

**Stato:** `APPROVATA`
**Funzione:** trasformare il problema nella necessità di CASA.  
**Priorità:** P1, P2, P3, P4, P5, P8.

**Problema del testo corrente**

- “Spazi dedicati” può significare uffici, aule o sale riunioni.
- Non distingue le risorse specialistiche esistenti dall'assenza dello spazio comune.
- Introduce CASA prima di averne fissato la gerarchia.

**Testo approvato**

#### Un luogo comune.

Ambiente, agricoltura, cibo e salute si influenzano ogni giorno.

Ma le risorse per studiarli e innovare restano separate.

Manca uno spazio dove metterle in relazione e farle lavorare insieme.

**Decisioni e osservazioni:** la pagina deve soltanto far percepire il vuoto. Spazio sperimentale, rete, infrastruttura digitale e regia vengono sviluppati nelle pagine successive.

### 05 · Per questo nasce CASA

**Stato:** `APPROVATA`
**Funzione:** fornire la definizione memorabile di CASA.  
**Priorità:** P1, P2, P3, P4, P5, P8.

**Problema del testo corrente**

- Spazio, rete e regia appaiono come tre componenti equivalenti.
- Lo spazio è descritto in modo più generico della rete.
- I quattro ambiti scompaiono proprio nella definizione.

**Testo approvato**

#### Il luogo dove le connessioni diventano innovazione.

CASA nasce come uno spazio sperimentale condiviso.

Qui i dati su clima, acqua e suolo si collegano alle pratiche agricole, alla qualità del cibo e ai suoi effetti sulla salute.

Formazione, sperimentazione e ricerca lavorano sulla stessa catena.

Le risorse dei partner ne ampliano le capacità. Una regia comune le mette in relazione.

**Decisioni e osservazioni:** la pagina mostra una connessione concreta tra i quattro ambiti. Configurazione, localizzazione e funzionamento dello spazio vengono sviluppati nelle pagine successive.

### 06 · Lo spazio CASA

**Stato:** `APPROVATA`
**Funzione:** rendere CASA concreta e immaginabile.  
**Priorità:** P1, P2, P3, P4, P5, P6, P8.

**Testo approvato**

#### Un ettaro di innovazione condivisa.

Un modello fisico, modulare e adattabile.

**Aree di sperimentazione e monitoraggio**

Campi, colture, percorsi sensorizzati e sistemi di osservazione per collegare clima, acqua, suolo, biodiversità e pratiche agricole.

**Ambienti controllati e laboratori**

Serre e moduli attrezzati per sperimentare coltivazioni, tecnologie, qualità e trasformazione del cibo e relazioni con la salute.

**Spazi per formare e condividere**

Ambienti utilizzabili per formazione, dimostrazioni, divulgazione ed eventi, nei quali rendere visibili attività, dati e risultati.

**Configurazione di riferimento**

Una prima configurazione è stata sviluppata su questa scala. La configurazione cambia con il luogo. La funzione resta: innovare insieme nei quattro ambiti.

**Decisioni e osservazioni:** il payoff dell'ettaro resta protagonista. “Configurazione di riferimento” chiarisce che si tratta di una scala progettuale già studiata, non di un terreno individuato o di un vincolo rigido.

### 07 · Il principio di CASA: un Internet delle risorse

**Stato:** `DA VERIFICARE`
**Funzione:** spiegare come lo spazio si collega alle capacità distribuite.  
**Priorità:** P1, P2, P3, P4, P5, P6, P8.

**Problema del testo corrente**

- “Senza concentrarle in un solo luogo” fa sembrare lo spazio non necessario.
- CASA rischia di tornare a essere soltanto una rete di partner.

**Testo approvato sul piano concettuale**

#### Un Internet delle risorse.

**Accesso, non possesso**

Imprese, ricercatori, professionisti e istituzioni possono utilizzare risorse che sarebbe difficile costruire o acquisire autonomamente.

**Le risorse dello spazio CASA**

Campi sperimentali, sensori, serre, laboratori e infrastrutture digitali vengono condivisi e resi accessibili secondo regole comuni.

**Le risorse dei partner**

Laboratori specialistici, dati, tecnologie e competenze che restano presso i partner vengono collegati allo stesso sistema di accesso.

Anche una piccola impresa può così combinare dati ambientali, sperimentazione agricola, analisi sul cibo e competenze sulla salute senza dover costruire da sola l'intera infrastruttura.

Lo spazio è il luogo dell'integrazione.

L'Internet delle risorse è il sistema che ne rende accessibili e ne estende le capacità.

**Decisioni e osservazioni:** il testo dovrà essere nuovamente valutato insieme alla grafica, perché parte della spiegazione potrà essere affidata allo schema o all'animazione. IoR condivide sia le risorse interne allo spazio CASA sia quelle distribuite presso i partner; il beneficio distintivo è l'accesso a capacità avanzate senza doverle possedere.

### 08 · Quattro ambiti che lavorano insieme

**Stato:** `DA RIVEDERE`  
**Funzione nuova:** non ridefinire CASA, ma dimostrare il ciclo concreto ambiente → agricoltura → cibo → salute → nuove evidenze.

**Priorità:** P1, P3, P4, P5, P6, P8.

**Proposta:** mantenere l'animazione e riscrivere il testo dopo l'approvazione delle pagine 04–07.

**Decisioni e osservazioni:** —

---

## Blocco C — Funzionamento

### 09 · Come opera CASA

**Stato:** `DA RIVEDERE`  
**Funzione:** mostrare il ciclo formazione → sperimentazione → sviluppo → misurazione e comunicazione.  
**Priorità:** P1, P2, P3, P4, P5, P6, P8.

**Intervento previsto:** ancorare ogni fase allo spazio e alle relazioni tra i quattro ambiti; evitare un catalogo di servizi.

### 10 · Per chi è CASA e benefici

**Stato:** `DA RIVEDERE`  
**Funzione:** destinatari, modalità di accesso e valore ottenuto.  
**Priorità:** P1, P2, P3, P4, P5, P8.

**Intervento previsto:** integrare destinatari e benefici; mostrare usi differenti della stessa infrastruttura.

---

## Blocco D — Credibilità

### 11 · Ecosistema e ruoli

**Stato:** `DA RIVEDERE`  
**Funzione:** mostrare chi rende possibile CASA.  
**Priorità:** P1, P2, P4, P5, P7, P8.

**Intervento previsto:** distinguere promotore, gestore, motore accademico, partner scientifici e sviluppi potenziali; collegare ogni ruolo allo spazio CASA.

### 12 · CASA non parte da zero

**Stato:** `DA RIVEDERE`  
**Funzione:** prova di capacità e fattibilità.  
**Priorità:** P1, P2, P4, P5, P7, P8.

**Intervento previsto:** sostituire l'elenco di esperienze/progetti con capacità già disponibili per attivare CASA.

### 13 · Roadmap

**Stato:** `DA RIVEDERE`  
**Funzione:** mostrare un percorso credibile di realizzazione.  
**Priorità:** P1, P2, P3, P4, P5, P7, P8.

**Intervento previsto:** distinguere definizione funzionale, criteri di localizzazione, individuazione e validazione dell'area, progettazione, predisposizione e attivazione; eliminare la sovrapposizione temporale del 2027.

### 14 · Direzione scientifica

**Stato:** `DA RIVEDERE`  
**Funzione:** autorevolezza e presidio scientifico.  
**Priorità:** P7, P8.

**Intervento previsto:** verificare stato dell'incarico e formulare correttamente il riferimento al Premio Nobel assegnato all'IPCC.

### 15 · Chiusura

**Stato:** `DA RIVEDERE`  
**Funzione:** fissare il ricordo finale.  
**Priorità:** tutte.

**Intervento previsto:** chiudere sul luogo nel quale i quattro ambiti lavorano insieme, non su una definizione generica di infrastruttura.

---

## Approfondimenti

### A1 · Accordo UNIUST–Università della Tuscia

**Stato:** `APPENDICE`  
**Nota:** pagina autonoma; verificare titolarità, docenza, iscrizioni congiunte, ricerca e vantaggi reciproci.

### A2 · Università di Malta

**Stato:** `APPENDICE`  
**Nota:** pagina autonoma; qualificare stato e funzione senza presentare come operativo ciò che è ancora in sviluppo.

### A3 · Europa, Mediterraneo e Piano Mattei

**Stato:** `APPENDICE`  
**Nota:** presentare come quadro di coerenza e sviluppo possibile, non come riconoscimento o finanziamento acquisito.

### A4 · Add-on InnovaliaTech

**Stato:** `APPENDICE`  
**Nota:** documento separato. L'investimento indicativo di 300.000 euro deve essere collegato a una configurazione funzionale, a criteri di localizzazione e a dotazioni chiaramente comprese o escluse.

---

## Registro delle decisioni

| Data | Pagina/tema | Decisione | Effetto sulle altre pagine |
|---|---|---|---|
| 12 luglio 2026 | Spazio CASA | Lo spazio fisico è necessario; la localizzazione non è ancora individuata. | Definizione, Internet delle risorse, roadmap e chiusura. |
| 12 luglio 2026 | Natura dello spazio | Non uffici: aree aperte, serre, campi sensorizzati, laboratori modulari, formazione ed eventi. | Cosa manca, definizione, funzionamento e benefici. |
| 12 luglio 2026 | Internet delle risorse | La rete amplia lo spazio comune e non lo sostituisce. | Definizione, ecosistema, patrimonio e roadmap. |
| 12 luglio 2026 | Metodo di revisione | Matrice priorità × pagine; riscrittura per blocchi con audit trasversali. | Intero processo editoriale. |
| 12 luglio 2026 | Pagina 04 | Approvato un testo breve che introduce il bisogno di un luogo comune senza anticipare la soluzione. | Le pagine 05–07 devono distribuire definizione, spazio, rete e regia senza ripetizioni. |
| 12 luglio 2026 | Concretezza dei quattro ambiti | Introdotto il test di sostituibilità: se un testo può descrivere un altro polo cambiando soltanto i nomi degli ambiti, è troppo generico. | Tutte le pagine devono mostrare relazioni concrete tra fenomeni, dati, pratiche o risultati. |
| 12 luglio 2026 | Pagina 05 | Approvata la definizione di CASA come spazio sperimentale che collega dati ambientali, pratiche agricole, qualità del cibo ed effetti sulla salute. | La pagina 06 deve rendere fisicamente immaginabile lo spazio; la 07 deve spiegare la rete. |
| 12 luglio 2026 | Pagina 06 | Approvato “Un ettaro di innovazione condivisa” come payoff; campi, serre e laboratori sono presentati come famiglie funzionali di un modello modulare. | La roadmap e l'add-on dovranno distinguere configurazione di riferimento, localizzazione e investimento. |
| 12 luglio 2026 | Pagina 07 | IoR rende accessibili e combinabili sia le risorse dello spazio CASA sia quelle dei partner; separa accesso e possesso. Testo da verificare con la grafica. | Benefici, destinatari e modello operativo devono esplicitare l'accesso delle piccole imprese a capacità altrimenti difficilmente raggiungibili. |

## Parcheggio — questioni non ancora decise

- Payoff definitivo della copertina.
- Presenza dell'ettaro nella presentazione generale o soltanto nell'add-on.
- Formulazione definitiva del ruolo di InnovaliaTech.
- Stato formale di CREA, CMCC e Fondazione Santa Lucia.
- Stato e formulazione della direzione scientifica.
- Perimetro definitivo di Malta e Piano Mattei nel materiale pubblico.
